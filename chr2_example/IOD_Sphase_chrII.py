...
# Initiating a list for storing IODs 
iod_list = []

# for finding IODs
# dictionary mapping each simulation number to a list of positions of origins which fired during that simulation
iteration2firePos_list = {}
...
for i in range(0,numberOfIterations):
    iteration2firePos_list[i]=[] # for each simulation, making a list to store the positions of origins which fire


maxRepTimes = [] # for recording the simulated lengths of S-phase
totalFireCount = [] # for tracking the number of origins which fire in each simualtion 
for index, iteration in enumerate(iterations):  # iterating over each itteration
    ...
    for line in iteration.splitlines():
        ...
        if len(splitLine) == 7 and splitLine[1] in ["factor"] and splitLine[5] in ["fire"]: # lines where origins fire
            ...
            iteration2firePos_list[index].append(firePos)
            oriFireCount+=1
    ...
    for i in range(len(firePos_list)-1):
        ...
        iod_list[chromosome].append(dist)
...
iod_df = pd.DataFrame({"iod":iod_list})
