...
bcsScriptName = '15FF0p05d_unfitted_mapOris_chrII.bc'  # to identify which version of the model is being used (eg: chromosome 2 model with 15 firing factors and recycling rates of 0.05)
...
#------------------ 05 EXTRACTING_ORI_PARAMETERS ------------------
    ...
    parameter_df = pd.DataFrame(parameter_data, columns=["i","fire","ori"]).astype(float)
    parameter_df['i'] = parameter_df['i','ori'].round().astype(int)
