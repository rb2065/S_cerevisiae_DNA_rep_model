...
simulations_per_run = 10 # number of model simulations per iteration of fitting
bcsScriptName = '200FF0p05d_mapOris'  # to identify which version of the model is being used (eg: model with 200 firing factors and recycling rates of 0.05)
...
folderPath = f'bcs_scripts/{new_folder}'
os.makedirs(folderPath, exist_ok=True)

# making a reset script (to save a copy of the original bcs script)
bcsScriptPath = f'bcs_scripts/{bcsScriptName}.bc'
...
#------------------ 05 EXTRACTING_ORI_PARAMETERS ------------------
    ...
    parameter_df = pd.DataFrame(parameter_data, columns=["i","ch","length","fire","ori"]).astype(float)
    parameter_df[['i','ch','length','ori']] = parameter_df[['i','ch','length','ori']].round().astype(int)
...
#------------------ 06 CHANGING_FIRING_RATES ------------------
    ...
    for chromosome, param_df in chr2parameter_df.items():
        ...
        for index, fire in enumerate(param_df['fire']):
            new_fire = fire*(ori_df.loc[index,'repTime_sim']/ori_df.loc[index,'expRepTime'])**1.2
...
#------------------ 08 ERROR ------------------
    ...
    for chromosome in range(1,17):
        fullTime_ext = chr2run2timeStore[chromosome][runs]
        fullTimeStore_list.extend(fullTime_ext)

    absolute_errors = abs(expRepTime_df['expRepTime']-fullTimeStore_list)
    squared_errors=(expRepTime_df['expRepTime']-fullTimeStore_list)**2
