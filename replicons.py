# This analysis the output from an alternative version of the model in which each fork has an additional parameter "ori" which is the position of the origin from which it initiated
...
bcsOutputName = "200FF0p05d_mapOris_fitted_s10" # 10 simulations of a fitted model with F=200 and recycling rates of 0.05 (It is recommended to generate and analyse a 500 simulation output)
...
# seting up a dictionary mapping each chromosome to a nested dictionary mapping each origin on that chromosome to a count of the number of times DNA is replicated by forks initiating from that origin (over all simulations)
chr2ori2replicons = {} 
